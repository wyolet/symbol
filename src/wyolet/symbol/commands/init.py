"""Init command — analyze a project and generate a recommended [tool.symbol] config."""
...
from wyolet.symbol.checkers.side_effects import SideEffect
from wyolet.symbol.shared.context import build_context
from wyolet.symbol.shared.findings import Report
from wyolet.symbol.shared.runner import run_checkers
...
def _generate_toml(
    ...
    lines: list[str] = []
    lines.append("[tool.symbol]")

    lines.append("")
    lines.append("[tool.symbol.checkers.orphans]")
    lines.append('severity = "warning"')

    lines.append("")
    lines.append("[tool.symbol.checkers.side_effects]")
    ...
    lines.append("")
    lines.append("[tool.symbol.checkers.unused_deps]")
    if ignore_deps:
...
def init_cmd(path: str) -> None:
    ...
    console.print(
        Panel(
            Text(f"  symbol init — analyzing {project_name}/", style="bold"),
            style="dim",
            expand=False,
        )
    )
