...
@dataclass(frozen=True)
class InitSpec:
    """symbol init command config from [init] in spec.toml."""
...
def load_spec(
    ...
    """
    from ca.symbol.shared.pkg_registry import normalize_package_name

    data_root = files("ca.symbol").joinpath("data")
