"""`ca delete-symbol` — CLI + rendering."""
...
def _render_agent(result: DeleteSymbolResult) -> None:
    ...
    if result.callers:
        ...
        print(f"Fix: ca callers {result.qualified_path.rsplit('.', 1)[-1]}")
        print(f"Then: ca patch <each file> to remove or rewrite each call site.")
...
def _render_rich(result: DeleteSymbolResult) -> None:
    ...
    if result.callers:
        ...
        console.print(t)
        console.print(
            f"\n[dim]Fix with:[/dim] [bold]ca patch <file>[/bold] [dim]for each[/dim]"
        )
