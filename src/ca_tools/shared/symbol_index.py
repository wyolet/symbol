...
from ca_tools.shared.symbol import (
...
class SymbolIndex:
    ...
    def stale_files(self) -> tuple[set[str], set[str]]:
        ...
        which means new files won't be auto-discovered until ``ca index``.
        """
        ...
        if current is None:
            ...
            # silently skipped on the incremental path; next ``ca index``
...
def _quick_is_clean(idx: "SymbolIndex") -> bool:
    ...
    Holes we accept for this tier of speed:
      ...
      * New file in a directory the index has never seen — rare for agents
        editing established projects; a manual ``ca index`` resolves it.
