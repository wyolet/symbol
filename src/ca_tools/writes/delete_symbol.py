"""`ca delete-symbol` — remove a named symbol from its file.

Single-file write. Uses the symbol index to resolve name → byte range, then
composes ca patch with empty content to do the actual splice. Refuses if
...
from ca_tools.writes.patch import PatchRequest, apply_patch
