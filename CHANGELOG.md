...
### Known limitations
- **`RenameSymbol` is textual (tier-1).** It matches the trailing identifier of selector expressions, so renaming `foo.Service.Save` also rewrites unrelated `bar.Service.Save()` calls that share the leaf name. The fix (AST-driven receiver-type inference, then `go/types` for full coverage) lands in a follow-up release. Use dry-run + manual review for cross-package method names like `Save`, `Get`, `Set`, `Close`.
