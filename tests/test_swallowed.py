...
from wyolet.symbol.checkers.swallowed import detect, SwallowedException
...
def _run(tmp_path: Path, cache: ASTCache) -> list[SwallowedException]:
    results = []
    for f in cache.files:
        results.extend(detect(None, f, cache.get_ast(f)))  # type: ignore[arg-type]
    return results
